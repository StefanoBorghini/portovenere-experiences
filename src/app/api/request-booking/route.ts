...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Prezzo per riga (esperienze + enhancement) ricalcolato SEMPRE
    // server-side, mai fidandoci di un totale passato dal client —
    // serve sia per il dettaglio prezzi nella mail di conferma, sia
    // per i reminder successivi (che leggono confirmed_selection.
    // experienceDetails/enhancementDetails, salvati qui sotto).
    ...
    if (!emailResult.success) {
      console.error("request-booking: verification email failed to send", emailResult.error);
    }

    // La richiesta di prenotazione e' gia' salvata su Proposal (update
    // sopra) a prescindere dall'esito dell'invio email — un timeout SMTP
    // transitorio non deve far vedere al cliente un errore generico su
    // una richiesta che in realta' e' andata a buon fine. Il client
    // (proposalClient.tsx) non legge expires_at dalla risposta di questa
    // route oggi — vedi nota sotto per il secondo intervento necessario
    // lato frontend.
    return NextResponse.json({
      success: true,
