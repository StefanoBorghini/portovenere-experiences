...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!proposal.email_verified) {
      ...
      // Dettaglio prezzi gia' calcolato server-side e salvato da
      // /api/request-booking al momento della richiesta — nessun
      // bisogno di ririsolvere nomi/prezzi qui.
      const experienceDetails = proposal.confirmed_selection?.experienceDetails || [];
      const enhancementDetails = proposal.confirmed_selection?.enhancementDetails || [];
