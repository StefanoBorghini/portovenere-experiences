...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // confirmed_selection si popola SOLO dopo che il cliente clicca
    // "Request Private Booking" (vedi /api/request-booking) — questo
    // reminder viene invece inviato PRIMA di quel momento, quindi va
    // quasi sempre vuoto. Quando manca, l'unica "selezione" reale
    // rimane la featured experience calcolata da generateProposal,
    // la stessa gia' mostrata nella mail di generazione — non tutto
    // il pacchetto disponibile, che il cliente non ha ancora scelto.
    const hasConfirmedSelection =
      (proposal.confirmed_selection?.experienceDetails?.length ?? 0) > 0;

    const featuredLineItem = hasConfirmedSelection
      ? null
      : await getFeaturedExperienceLineItemForLead({
    ...
    const summaryData = {
      ...
      slug: proposal.slug,
      experienceDetails: hasConfirmedSelection
        ? proposal.confirmed_selection?.experienceDetails || []
        : featuredLineItem
        ? [featuredLineItem]
        : [],
      enhancementDetails: proposal.confirmed_selection?.enhancementDetails || [],
      totalPrice: hasConfirmedSelection
        ? proposal.total_price || 0
        : featuredLineItem?.price ?? undefined,
    };
