...
export async function getProposalForLead(leadId: string) {
  ...
  // scelto davvero e se ha confermato l'email. payment_status/
  // concierge_fee_amount/concierge_fee_percentage servono per il
  // badge "Concierge Active" e per la sezione "Operator coordination"
  // (vedi src/app/admin/leads/[id]/page.tsx).
  const { data, error } = await supabase
    .from("Proposal")
    .select(
      "slug, expires_at, total_price, email_verified, confirmed_selection, proposal_data, payment_status, concierge_fee_amount, concierge_fee_percentage"
    )
    .eq("lead_id", leadId)
    .maybeSingle();
